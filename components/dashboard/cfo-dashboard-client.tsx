...
interface KPI {
  id: string; label: string; value: number; prev: number;
  unit: "crore" | "pct" | "months"; goodWhenUp: boolean;
}
interface SummaryRow {
  id: string; category: string; budget: number; actual: number;
  variance: number; variancePct: number;
}
interface DeptRow {
  id: string; department: string; budget: number; actual: number; variance: number;
}
interface LineItemRow {
  id: string; lineItem: string; budget: number; actual: number;
  variance: number; notes: string;
}

function fyMonths(startYear: number): string[] {
  const months = ["Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar"];
  return months.map((m, i) => `${m} ${String(i <= 8 ? startYear : startYear + 1).slice(2)}`);
}
...
const kpis: KPI[] = [
  { id:"revenue",  label:"Total Revenue",     value:142_50_00_000, prev:128_00_00_000, unit:"crore",  goodWhenUp:true  },
  { id:"ebitda",   label:"EBITDA",             value:28_40_00_000,  prev:24_10_00_000,  unit:"crore",  goodWhenUp:true  },
  { id:"ebitda_m", label:"EBITDA Margin",      value:19.9,          prev:18.8,          unit:"pct",    goodWhenUp:true  },
  { id:"cash",     label:"Cash & Equivalents", value:54_60_00_000,  prev:61_20_00_000,  unit:"crore",  goodWhenUp:true  },
  { id:"burn",     label:"Monthly Burn",       value:4_20_00_000,   prev:3_90_00_000,   unit:"crore",  goodWhenUp:false },
...
const plTrend = MONTHS.map((m, i) => ({
  month: m,
  revenue:  Math.round((110 + i * 2.8 + Math.sin(i) * 3) * 1e7),
  expenses: Math.round((88  + i * 1.9 + Math.cos(i) * 2) * 1e7),
  ebitda:   Math.round((22  + i * 0.9 + Math.sin(i) * 1) * 1e7),
}));

const cashFlowData = MONTHS.map((m, i) => ({
  month: m,
  operating: Math.round((18 + Math.sin(i * 0.8) * 4) * 1e7),
  investing: Math.round((-8 - Math.cos(i * 0.5) * 2) * 1e7),
  financing: Math.round((2  + Math.sin(i * 1.2) * 1) * 1e7),
}));
